...
const accountingFile = 'src/AccountingModule.tsx';
const reportFile = 'src/AccountingReports.tsx';
const marker = '// v88-move-accounting-reports';

let app = fs.readFileSync(appFile, 'utf8');
let accounting = fs.readFileSync(accountingFile, 'utf8');
let changed = false;
...
if (!fs.existsSync(reportFile)) {
  fs.writeFileSync(reportFile, `import { useEffect, useMemo, useState } from 'react';
...
if (!app.includes(marker)) {
  app = replaceOnce(app, "import FinancialStatements from './FinancialStatements';", "import FinancialStatements from './FinancialStatements';\nimport AccountingReports, { type AccountingReportMode } from './AccountingReports';", 'FarmApp accounting reports import');
  app = replaceOnce(app, "const reportPages = ['hub', 'financial', 'tbs', 'purchases', 'receivables', 'inventory', 'assets'] as const;", "const reportPages = ['hub', 'financial', 'accounting', 'tbs', 'purchases', 'receivables', 'inventory', 'assets'] as const;", 'FarmApp report pages');

  const financialState = `  const openFinancialReport = (nextMode: (typeof reportModeOptions)[number]) => {\n    changeReportMode(nextMode);\n    changeReportPage('financial');\n  };`;
  app = replaceOnce(app, financialState, `${financialState}\n\n  ${marker}\n  const accountingReportModeStorageKey = 'perkebunan.navigation.reports.accounting';\n  const accountingReportModeOptions = ['journals', 'ledger', 'trial'] as const;\n  const [accountingReportMode, setAccountingReportMode] = useState<AccountingReportMode>(() => readStoredChoice(accountingReportModeStorageKey, accountingReportModeOptions, 'journals'));\n  const openAccountingReport = (nextMode: AccountingReportMode) => {\n    setAccountingReportMode(nextMode);\n    storeChoice(accountingReportModeStorageKey, nextMode);\n    changeReportPage('accounting');\n  };`, 'FarmApp accounting report state');
  ...
  app = replaceOnce(app,
    ...
    'FarmApp accounting report card');

  app = replaceOnce(app,
    "    financial: 'Laporan Keuangan',",
    "    financial: 'Laporan Keuangan',\n    accounting: 'Laporan Akuntansi',",
    'FarmApp accounting page title');
  ...
  app = replaceOnce(app, financialDetail, financialDetail + accountingDetail, 'FarmApp accounting report detail');

  fs.writeFileSync(appFile, app);
  changed = true;
}

if (!accounting.includes(marker)) {
  accounting = replaceOnce(accounting,
    "  const [mode, setMode] = useState<AccountingMode>(() => readStoredChoice(storageKey, accountingModes, 'hub'));",
    ...
    'AccountingModule stored report mode cleanup');
  accounting = accounting.replace('Setup akun, jurnal, buku besar dan kontrol saldo.', 'Setup akun, jurnal penyesuaian, akun penting, saldo awal, dan periode.');
  accounting = accounting.replace(`<button type="button" className="master-hub-card" onClick={() => changeMode('journal')}><span className="master-hub-card-icon"><BookOpen size={24} /></span><span className="master-hub-card-copy"><strong>Jurnal Umum</strong><small>Lihat jurnal otomatis dan jurnal penyesuaian pada satu daftar.</small></span><ChevronRight size={18} /></button>\n        `, '');
  accounting = accounting.replace(`<button type="button" className="master-hub-card" onClick={() => changeMode('ledger')}><span className="master-hub-card-icon"><Landmark size={24} /></span><span className="master-hub-card-copy"><strong>Buku Besar</strong><small>Mutasi Debit/Kredit dan saldo berjalan per akun Level 4.</small></span><ChevronRight size={18} /></button>\n        `, '');
  accounting = accounting.replace(`<button type="button" className="master-hub-card" onClick={() => changeMode('trial')}><span className="master-hub-card-icon"><Scale size={24} /></span><span className="master-hub-card-copy"><strong>Neraca Saldo</strong><small>Kontrol saldo Debit/Kredit seluruh akun hingga akhir periode.</small></span><ChevronRight size={18} /></button>\n`, '');
  accounting = accounting.replace('<strong>Jurnal Manual</strong><small>Input jurnal penyesuaian yang tidak berasal dari modul operasional.</small>', '<strong>Jurnal Umum / Penyesuaian</strong><small>Input jurnal umum dan penyesuaian yang tidak berasal dari modul operasional.</small>');
  accounting = accounting.replace("mode === 'manual' ? 'Jurnal Manual'", "mode === 'manual' ? 'Jurnal Umum / Penyesuaian'");
  accounting = accounting.replace("'Edit Jurnal Manual' : 'Jurnal Manual / Penyesuaian'", "'Edit Jurnal Umum / Penyesuaian' : 'Jurnal Umum / Penyesuaian'");
  accounting = accounting.replace('Riwayat Jurnal Manual', 'Riwayat Jurnal Umum / Penyesuaian');
  fs.writeFileSync(accountingFile, accounting);
  changed = true;
}

console.log(changed ? 'Applied v88 move accounting reports' : 'v88 already applied');
